...
# ---------------------------------------------------------------------------
# parser_hints.json — auto-generated by the ML pipeline (Phase 4).
# Merged into _PRIMARY_FOR and format-detection at import time.
# Safe to delete: the parser falls back to its built-in tables gracefully.
# ---------------------------------------------------------------------------
_HINTS_PATH = Path(__file__).parent / "parser_hints.json"
_PARSER_HINTS: dict = {}

def _load_parser_hints() -> dict:
    """Load parser_hints.json once at startup; returns {} if missing/broken."""
    global _PARSER_HINTS
    if _PARSER_HINTS:
        return _PARSER_HINTS
    try:
        _PARSER_HINTS = json.loads(_HINTS_PATH.read_text(encoding="utf-8"))
        log.info("parser_hints.json loaded: %d formats, %d alias tables",
                 len(_PARSER_HINTS.get("format_record_counts", {})),
                 sum(len(v) for v in _PARSER_HINTS.get("field_aliases", {}).values()))
    except FileNotFoundError:
        pass
    except Exception as exc:
        log.warning("Could not load parser_hints.json (%s) — using built-in tables only", exc)
    return _PARSER_HINTS
...
def _detect_format(filename: str, text: str) -> str:
    ...
    # ML-derived format signals from parser_hints.json (Phase 4 pipeline).
    # If a format's distinctive bigrams dominate the sample, use that format.
    hints = _load_parser_hints()
    fmt_signals = hints.get("format_detection_signals", {})
    if fmt_signals and sample_lines:
        sample_text = " ".join(sample_lines).lower()
        best_fmt, best_score = None, 0
        for fmt, signals in fmt_signals.items():
            if fmt in ("generic", "csv"):
                continue  # too broad to use as a positive signal
            hits = sum(1 for sig in signals if sig["pattern"] in sample_text)
            score = hits / max(len(signals), 1)
            if score > best_score and score >= 0.3:
                best_fmt, best_score = fmt, score
        if best_fmt:
            log.debug("Format hint from parser_hints: %s (score=%.2f)", best_fmt, best_score)
            return best_fmt
...
def _apply_aliases(df: pd.DataFrame, fmt: str = "") -> pd.DataFrame:
    """
    ...
    Also merges corpus-learned aliases from parser_hints.json (Phase 4 ML pipeline)
    so newly discovered field names are automatically handled without code changes.
    """
    hints = _load_parser_hints()
    # Build a merged alias lookup: hints aliases take second priority after _PRIMARY_FOR
    # We invert the hints {field: target} map into {target: [field, ...]}
    extra_candidates: Dict[str, List[str]] = {}
    hint_aliases = hints.get("field_aliases", {})
    for scope in ([fmt] if fmt else []) + ["json", "logfmt", "csv", "syslog"]:
        for field, target in hint_aliases.get(scope, {}).items():
            extra_candidates.setdefault(target, [])
            if field not in extra_candidates[target]:
                extra_candidates[target].append(field)
    ...
    for target, candidates in _PRIMARY_FOR.items():
        ...
        for cand in list(candidates) + extra_candidates.get(target, []):
